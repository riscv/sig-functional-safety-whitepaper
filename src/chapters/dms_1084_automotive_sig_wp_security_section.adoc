:version: 0.1
:project-name: Automotive SIG Cybersecurity Section
...
* 0.2
** Update to align with Thomas Roecker (Infineon), Sam Visalli (Tenstorrent) and Richard York (SiFive) comments
...
** The M-mode environment is reserved for a minimal task scheduler or compact RTOS and critical hardware abstraction firmware. Adopting micro-kernel architectures can further reduce this footprint by delegating driver execution to U-mode.
