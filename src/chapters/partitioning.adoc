...
Beware, that while minimal, there might be some differences between the final
ratified version of these specifications and the text in these sections.
The text will be updated as these specifications get ratified.
...
The IOPMP specification, currently near ratification, introduces a Request Role
Identifier (RRID) for each bus initiator.
...
Similarly, the group working on the [.extension]#Sspmp# extension has already
envisioned its extension to provide hypervisor support by defining a dual-stage
PMP  cite:[shspmp:2025].
...
// Supervisor Domain Access Protection (SmMTT) cite:[smmtt:2025] is a
...
The under development Supervisor Domain Access Protection (SmMTT)
specification cite:[smmtt:2025] defines a RISC-V privileged architecture
...
Regarding the under development SmMTT specification, there are
...
However, as pointed in <<sec:caches:recom:isa>>, the developer should be aware
...
// when available, but if required then the user should consider enabling the IOMMU
// `T2GPA` control bit (see the RISC-V IOMMU cite:[rviommu:2025] for further
// details) or alternatively, when available, using other means provided by the
// SoC to ensure the memory partititioning enforcement, like the usage of IOPMPs.
PCIe ATS (Address Translation Services) support should be disabled by default
when available, but if required then the user should consider enabling the IOMMU
`T2GPA` control bit (see the RISC-V IOMMU cite:[rviommu:2025] for further
details) or alternatively, when available, using other means provided by the
SoC to ensure the memory partititioning enforcement, like the usage of the
...
IOPMP is an under development RISC-V specification providing PMP like memory
