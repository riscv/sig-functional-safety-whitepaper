...
Memories (TIMs), or local store) are important components, particularly in high
...
the processor to reduce the number of accesses to main memory, which can improve
...
may have widely differing access times, depending upon cache state that is not
easily predictable or controllable by software.
* Concurrent access to different memory areas from the same core (e.g. a
superscalear core with multiple cache ports) or different cores may be cached
...
* The cache usage behavior of one core may affect the processes' performance and
execution time variability in another core.
This may be due to the above mentioned cache state and concurrent access, but
also due to the caches capabilities to handle the cores' requests, e.g.
due to the maximum number of misses the cache can handle a core might need to
wait for the misses of another core to be serviced before handling its requests.
...
design, the requirements to achieve this may be non-trivial.
...
Functionally safe systems must be able to provide enough temporal isolation (a
lack of interference of a software task by the previous state of the cache) or
sufficient temporal guarantees to satisfy the intended use-case's requirements.
Thus, functionally safe systems should address at hardware, software, or through
a combination of both the aforementioned issues when using caches.
...
A TCM that is shared may implement mechanisms to provide isolation, such as
...
discussed in this chapter, it is recommended to perform a cache state analysis
(probably an offline analysis) to selectively issue the [.extension]#Zicbom#
instructions required to achieve the cache reset instead of just issuing
[.extension]#Zicbom# instructions for the entire context, e.g. an application,
address space.
The latter might have an important negative performance impact when the size
of the cache blocks is small compared to the entire context address space size.
...
loss.
We have identified two standard ways for globally disabling and enabling the
caches with the current RISC-V specifications:
+
--
...
--
+
Other ways may exist, be proposed by future extensions or provided by
